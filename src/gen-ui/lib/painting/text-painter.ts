  ...
  implements TextStyleOption, TextDecorationOption
{
  ...
  public getTextStyle(style?: Partial<TextStyleOption>): TextStyle {
    return this.copyWith(style);
  }
  public apply(style?: Partial<TextStyleOption>): TextStyle {
    return this.copyWith(style);
  }
  private toOption(): Required<TextStyleOption> {
    return {
      color: this.color,
      fontSize: this.fontSize ?? _kDefaultFontSize,
...
export class Paragraph extends ParagraphControl {
   ...
   */
  public performLayoutTextOffset(
    paint: Painter,
    startOffset: Offset,
    constraints: ParagraphConstraints
  ) {
  ...
  private performPaint(paint: Painter, offset: Offset, debugRect: boolean) {
    ...
    while (child != null) {
      ...
      const baseLine = currentY - lineHeight * 0.5;
      let baselineY =
        baseLine + this.textStyle.fontSize * 0.3 + parentData.baseLineOffsetY;
