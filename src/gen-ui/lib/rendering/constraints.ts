...
import { ArgumentError, AssertionError, StateError } from "../core/errors";
...
class BoxConstraints extends Constraints {
  ...
  constructor(
    ...
    if (this.minWidth > this.maxWidth || this.minHeight > this.maxHeight) {
      throw new ArgumentError("BoxConstraints are invalid: min > max.");
    }
  }
  ...
  public constrain(size: Size): Size {
    ...
    if (_size.width < 0 || _size.height < 0) {
      throw new StateError(`BoxConstraints collapsed: width=${_size.width}, height=${_size.height}`);
    }
    return _size;
  }
